...
void APP_BLE_Procedure_Gap_Central(ProcGapCentralId_t ProcGapCentralId)
{
  ...
  switch(ProcGapCentralId)
  {
    case PROC_GAP_CENTRAL_SCAN_START:
    {
      status = aci_gap_set_scan_configuration(DUPLICATE_FILTER_DISABLED, 0x00, LE_CODED_PHY_BIT, HCI_SCAN_TYPE_ACTIVE, paramA, paramB);
      if (status != BLE_STATUS_SUCCESS)
      {
      ...
      status = aci_gap_start_procedure (GAP_GENERAL_DISCOVERY_PROC,LE_CODED_PHY_BIT,0,0);
      if (status != BLE_STATUS_SUCCESS)
      {
...
static uint8_t analyse_ext_adv_report(packed_Extended_Advertising_Report_t *p_ext_adv_report)
{
  ...
  memcpy(data_temp, p_ext_adv_report, 60);
  ...
  else
  {
	...
	uint8_t payload_point = 0;
    uint8_t payload[p_ext_adv_report->Data_Length];
    memcpy(payload, &p_ext_adv_report->Data, p_ext_adv_report->Data_Length);

    // Check flags
    uint8_t flags_len = payload[payload_point];
    payload_point++;
    uint8_t type = payload[payload_point];
    payload_point++;
    if(type = 0x01)
    {
    	uint8_t value = payload[payload_point];
    	payload_point++;
    	if(value == 0x06)
    	{
	...
	// Decode Complete Local Name
    uint8_t name_len = payload[payload_point];
    payload_point++;
    type = payload[payload_point];
    payload_point++;
    uint8_t name[name_len-1];

	if(type == 0x09) // Complete Local Name
	{
		memcpy(name, &payload[payload_point], name_len-1);
		payload_point += name_len-1;
	}

	// Decode Manufacturer Specific Data
	uint8_t manuf_len = payload[payload_point];
	payload_point++;
	type = payload[payload_point];
	payload_point++;
	uint8_t manuf_data[manuf_len-1];

	if(type == 0xFF) // Manufacturer Specific Data
	{
		memcpy(manuf_data, &payload[payload_point], manuf_len-1);
		payload_point += manuf_len-1;
	}

	l_buff_pos += snprintf(&l_buff[l_buff_pos], L_BUFF_SIZE - l_buff_pos, "%s | ", name);
	l_buff_pos += snprintf(&l_buff[l_buff_pos], L_BUFF_SIZE - l_buff_pos, "%s | ", manuf_data);
    ...
    if( data_temp[44] == AD_TYPE_MANUFACTURER_SPECIFIC_DATA) // extended
    {
    	uint8_t length_man_specific_data = data_temp[45];

    	uint8_t custom_data[10];

    	memcpy(custom_data, data_temp[46], 10);
