...
//todo	INV_MSG(INV_MSG_LEVEL_WARNING, "DynProtocol: returned payload is -1");
...
static int DynProtocol_doProcess(DynProtocol_t * self)
{
	...
	default:
//todo		INV_MSG(INV_MSG_LEVEL_WARNING, "DynProtocol: Unexpected packet type");
		return -1;
	}
...
//todo		INV_MSG(INV_MSG_LEVEL_ERROR, "DynProtocol: Unexpected packet received.");
	}
...
//todo		INV_MSG(INV_MSG_LEVEL_ERROR, "DynProtocol: Unknown frame");
...
//todo			INV_MSG(INV_MSG_LEVEL_ERROR, "DynProtocol: Frame size error. size=%dB (expected %dB)", self->decode_state_machine.current_frame_size, self->decode_state_machine.expected_size + 2);
			return INV_ERROR_SIZE;
		}
	}
...
int DynProtocol_processPktByte(DynProtocol_t * self, uint8_t rcvByte)
{
	...
	case PROTOCOL_STATE_GID:
	{
		...
		if(self->decode_state_machine.group_id != DYN_PROTOCOL_GROUP_ID) {
			self->decode_state_machine.state  = PROTOCOL_STATE_IDLE;
//todo			INV_MSG(INV_MSG_LEVEL_ERROR, "DynProtocol: Invalid group ID");
			return -1;
		}
		break;
	}
	...
	case PROTOCOL_STATE_PAYLOAD:
	{
		...
		if(self->decode_state_machine.received_size >= MAX_EXPECTED_PAYLOAD)
		  {
		    //todo			INV_MSG(INV_MSG_LEVEL_WARNING, "DynProtocol: internal buffer size full");
		...
		uint8_t * outBuffer, uint16_t maxBufferSize, uint16_t *outBufferSize)
{
	...
	*outBufferSize = 0;
	...
	default:
//todo		INV_MSG(INV_MSG_LEVEL_ERROR, "DynProtocol: Unexpected argument for encode_command()");
		return -1;
	}
...
error_size:
//todo	INV_MSG(INV_MSG_LEVEL_ERROR, "DynProtocol: output buffer size too small");
	return -1;
}
...
error_arg:
//todo	INV_MSG(INV_MSG_LEVEL_ERROR, "DynProtocol: Unexpected argument for DynProtocol_encodeResponse()");
	return -1;
...
error_arg:
//todo	INV_MSG(INV_MSG_LEVEL_ERROR, "DynProtocol: Unexpected argument for encode_async()");
	return -1;
