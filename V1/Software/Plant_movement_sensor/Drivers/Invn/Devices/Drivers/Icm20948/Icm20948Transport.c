...
//			result |= inv_icm20948_read_reg(s, REG_MEM_R_W, dat, thisLen);

//			if (printReadMemsIMURegContent)
//			{
//              waitToPrint();
//              npf_snprintf(uart_buf, 200, "Icm20948Transport.c read_reg, data = dat[0]: %u, 0x%02X; dat[1]: %u, 0x%02X; dat[2]: %u, 0x%02X; dat[3]: %u, 0x%02X.\r\n", dat[0], dat[0], dat[1], dat[1], dat[2], dat[2], dat[3], dat[3]);
//              huart2print(uart_buf, strlen(uart_buf));
//			}
...
//		    if (!result){
//			waitToPrint();
//			npf_snprintf(uart_buf, 100, "Read reg REG_MEM_R_W successful. result= %d.\r\n", result);
//			huart2print(uart_buf, strlen(uart_buf));
//		    }
